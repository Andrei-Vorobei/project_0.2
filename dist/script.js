...
var drop = function drop() {
  ...
  drag *
  dragend *
  dragenter - объект входи в зону dropArea
  dragexit *
  dragleave - объект покидает зону dropArea
...
var filter = function filter() {
  ...
  menu.addEventListener('click', function (e) {
    ...
    if (target && target.tagName == "LI") {
      ...
      no.style.display = 'none';
      no.classList.remove('fadeIn');
      var classBtn = target.classList.value;
...
var forms = function forms() {
  ...
  var message = {
    loading: 'Загрузка...',
    success: 'Спасибо! Скоро мы с Вамм свяжемся',
...
var modals = function modals() {
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        giftTrigger.style.right = "".concat(+getComputedStyle(giftTrigger).right.replace(/\D/g, '') + calcScroll(), "px");
      });
    });
  ...
  function openByScroll(selector) {
    window.addEventListener('scroll', function () {
      var scrollHeght = Math.max(document.documentElement.scrollHeight, document.body.scrollHeight);

      if (!btnPressed && window.pageYOffset + document.documentElement.clientHeight >= scrollHeght) {
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  ...
  btn.addEventListener('click', function () {
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_6__["getResource"])('assets/db.json').then(function (res) {
